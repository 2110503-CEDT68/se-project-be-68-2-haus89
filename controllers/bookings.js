  ...
  try {
    ...
    if (existingBooking) {
      return res.status(400).json({
        success: false,
        message:
          "You already have a booking. Please delete or update your existing booking first.",
      });
    }
...
exports.getBooking = async (req, res, next) => {
  try {
    ...
    // Check if user owns this booking, is the assigned dentist, or is admin
    const isOwner = booking.user._id.toString() === req.user.id;
    const isAssignedDentist = booking.dentist._id.toString() === req.user.id;
    const isAdmin = req.user.role === "admin";

    if (!isOwner && !isAssignedDentist && !isAdmin) {
...
exports.updateBooking = async (req, res, next) => {
  try {
    ...
    const isDentistChanged =
      req.body.dentist && req.body.dentist !== booking.dentist.toString();
    const isDateChanged =
      req.body.date &&
      new Date(req.body.date).getTime() !== new Date(booking.date).getTime();
    const isTimeChanged =
      (req.body.startTime && req.body.startTime !== booking.startTime) ||
    ...
    if (isDentistChanged || isDateChanged || isTimeChanged) {
      ...
      const newDentist = await User.findOne({
        _id: newDentistId,
        role: "dentist",
      });
      if (!newDentist) {
      ...
      const availableSlot = newDentist.availableSlots.find((slot) => {
        ...
        slotDate.setHours(0, 0, 0, 0);
        return (
          slotDate.getTime() === newBookingDate.getTime() &&
      ...
      const oldSlot = oldDentist.availableSlots.find((slot) => {
        ...
        slotDate.setHours(0, 0, 0, 0);
        return (
          slotDate.getTime() === oldBookingDate.getTime() &&
...
exports.checkAvailability = async (req, res, next) => {
  try {
    ...
    const availableSlots = (dentist.availableSlots || []).filter(
      (slot) => !slot.isBooked,
    );
